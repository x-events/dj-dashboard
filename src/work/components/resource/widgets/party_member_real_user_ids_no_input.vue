  ...
  <div class="el-form-item"
    ...
    <div class="el-form-item__content" :style="{marginLeft: labelwidth+'px'}">
      <template v-if="disabled">
          <span v-for="(user, i) in user_list" :key="i"><span class="dib_bg" :title="user.name || user.realname">{{user.name || user.realname}}</span></span>
      </template>
      <template v-else>
        <div class="heightLayout">
            <div class="cont">
                <el-tag v-for="(user, i) in user_list" :key="'user'+i" size="medium" :closable="!disabled" type="info" @close="remove(i)">{{user.name || user.realname}}</el-tag>
            </div>
            <div class="_actions">
