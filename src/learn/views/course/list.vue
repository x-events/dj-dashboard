<template>
  <div class="scrollbar blockLayout" v-loading="loading">
    <div class="flex-row align-center toolBar">
        ...
        <el-button v-has="port.courseSave" type="primary" size="small" icon="el-icon-plus" @click="createNew()">新增</el-button>
    </div>
      ...
      <ul v-if="list.length">
        <li v-for="(e,i) in list" :key="i" class="flex-row">
          ...
          <div class="flex-1">
            ...
            <div class="duration">时长：{{e.minute}}分{{e.second}}秒</div>
            <div class="credit">学时：{{e.credit}}</div>
          </div>
          <div class="options flex-col">
            <div class="flex-1">
              <el-button v-has="port.courseSave" size="mini" type="primary" plain @click="handleEdit(e)">编辑</el-button>
              <el-button v-has="port.courseDel" size="mini" type="danger" plain @click="handleStatus(e,'0')">删除</el-button>
            </div>
            <div class="tr">
              <el-button v-has="port.courseDel" size="mini" :type="e.status == 1 ? 'danger':'primary'" @click="handleStatus(e,e.status)" v-if="e.status">{{e.status == 1 ?'下架':'上架'}}</el-button>
            </div>
          </div>
        </li>
      </ul>
    ...
    <!-- 编辑弹窗 -->
    <el-dialog :title="dialogTitle+'课程'" :visible.sync="dialogVisible" width="700px" @close="closeDialog">
      <course-editor 
        :data="editForm"
...
export default {
  components: {
    "course-editor": (resolve) => require(['@/learn/components/course-editor.vue'], resolve)
  },
  data() {
    return {
      port,
  ...
  methods: {
    ...
    handleStatus(item,ope){
      ...
      this.$confirm('是否继续?').then(() => {
        ...
        port.courseDel.r(this.delForm).then((res) => {
    ...
    fetchData() {
      this.loading = true;
      port.courseLists.r(this.queryParam).then(res => {
        let result = res.data;
        this.loading = false;
